...
class PresenceService {
    ...
    async setUserOnline(userId, socketId) {
        ...
        await this.redis.hset(`user:${userId}:presence`, 'lastSeen', new Date().toISOString());
    }
    ...
    async isUserActiveInConversation(userId, conversationId) {
        try {
            const isActive = await this.redis.sismember(`conversation:${conversationId}:active_users`, userId);
            return isActive === 1;
        } catch (error) {
